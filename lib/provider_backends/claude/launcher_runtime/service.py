...
def build_start_cmd(
    ...
    *,
    ...
    if profile is None or bool(getattr(profile, 'inherit_auth', True)):
        ...
        # A managed Claude process inherits a private credential copy. Disable
        # /login and /logout so neither command can reach an ambient OS
        # credential backend and mutate the user's external login.
        # Authentication changes are made outside CC_BRIDGE and inherited again
        # on the next managed start.
        managed_env.update(
            {
